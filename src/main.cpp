...
void rotate(int angle,int mode){
  ...
  if(angle > 0){
    ...
    while(true){
      ...
      || abs(origin - n_angle) < ALLOW_DIF)break;
      if(abs(target - n_angle) < ALLOW_DIF
      || abs(origin - n_angle) < ALLOW_DIF){
      color_read();
      judge_color();
      if(Line_Sensor[1] == BLACK || Line_Sensor[3] == BLACK)break;
      }
    }
  }else if(angle < 0){
    //反時計回り
